...
<div class="container-fluid mt-3">
    ...
    <table class="table table-no-border table-striped" style="margin: 0 auto;" id="table">
      ...
      <tbody>
        ...
        while ($row = $result->fetch_assoc()) {
          ...
          echo "</td>";
          echo "<td>";
            echo "<div style=\"display: flex; align-items: center;\">"; // Contenedor para alinear elementos verticalmente
            echo "<input type=\"password\" id=\"contrasena-{$row['id_User']}\" value=\"{$row['pass_User']}\" disabled style=\"width: 300%;\">"; // Campo de contraseña
            echo "<button type=\"button\" class=\"btn btn-info btn-sm\" onclick=\"showModalAndPassUserId('{$row['id_User']}')\"> <i class='fa-regular fa-eye' style='color: #ffffff;'></i> </button>"; // Botón para mostrar la contraseña
            echo "</div>";
          echo "</td>";
  ...
  while ($row = $result->fetch_assoc()) {
    ...
    echo "          <input type=\"text\" id=\"nuevo_nombre\" class=\"capitalize-input\" name=\"nuevo_nombre\" style=\"border-radius: 45px;\" pattern=\"^[A-ZÁÉÍÓÚÜÑ][a-záéíóúüñ]*(?: [A-ZÁÉÍÓÚÜÑ][a-záéíóúüñ]*)?$\" title=\"Ingresar solamente letras y/o espacios\"   value=\"{$row['nom_User']}\"><br><br>";

    echo "          <label for=\"nuevo_apPaterno\">Apellido paterno:</label>";
    echo "          <input type=\"text\" id=\"nuevo_apPaterno\" class=\"capitalize-input\" name=\"nuevo_apPaterno\" style=\"border-radius: 45px;\" pattern=\"^[A-ZÁÉÍÓÚÜÑ][a-záéíóúüñ]*(?: [A-ZÁÉÍÓÚÜÑ][a-záéíóúüñ]*)?$\" title=\"Ingresar solamente letras y/o espacios\"   value=\"{$row['ap_PatU']}\"><br><br>";
    
    echo "          <label for=\"nuevo_apMaterno\">Apellido materno:</label>";
    echo "          <input type=\"text\" id=\"nuevo_apMaterno\" class=\"capitalize-input\" name=\"nuevo_apMaterno\" style=\"border-radius: 45px;\" pattern=\"^[A-ZÁÉÍÓÚÜÑ][a-záéíóúüñ]*(?: [A-ZÁÉÍÓÚÜÑ][a-záéíóúüñ]*)?$\" title=\"Ingresar solamente letras y/o espacios\"   value=\"{$row['ap_MatU']}\"><br><br>";
    ...
    echo "          <input type=\"text\" id=\"nuevo_tel\" class=\"format-phone\" name=\"nuevo_tel\" style=\"border-radius: 45px;\"  title=\"Proporcione un número de teléfono válido de 10 dígito\"  maxlength=\"12\" pattern=\"^\d{3} \d{3} \d{4}$\" value=\"{$row['tel_User']}\"><br><br>";
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="showPasswordModalLabel">Ingrese su contraseña para validar que es usted.</h5>
      ...
      <div class="modal-body">
        <form id="verifyPasswordForm">
          ...
          <button type="submit" class="btn btn-primary" onclick="clearInput()">Verificar</button>
        </form>
        <div id="passwordError" style="display: none; color: red;">La contraseña de administrador es incorrecta.</div>
      </div>
    </div>
  </div>
</div>
...
<script>
    $(document).ready(function() {
        ...
        $('#nuevo_nombre').on('input', function() {
            validateInput($(this), 'Por favor, ingresa solamente letras y/o espacios.');
        });

        // Llama a la función de validación para el apellido paterno
        $('#nuevo_apPaterno').on('input', function() {
            validateInput($(this), 'Por favor, ingresa solamente letras y/o espacios.');
        });

        // Llama a la función de validación para el correo electrónico
        $('#nuevo_correo').on('input', function() {
...
<script>
  ...
  $(document).ready(function() {
    ...
    function showUserPassword(userId) {
        // Encuentra el elemento de contraseña en la fila correspondiente
        var contrasenaElement = $("#contrasena-" + userId);

        // Muestra la contraseña cambiando el tipo de input de 'password' a 'text'
        contrasenaElement.attr('type', 'text');

        // Después de unos segundos, vuelve a ocultar la contraseña cambiando el tipo nuevamente a 'password'
        setTimeout(function() {
            contrasenaElement.attr('type', 'password');
        }, 10000); // Cambia el tiempo en milisegundos según tus preferencias
    }
    ...
    var userId = $("#showPasswordModal").data('userId');

    // Hacer una solicitud al servidor para verificar la contraseña del administrador
    $.ajax({
    url: './pass_admin.php', // URL de tu endpoint para verificar la contraseña
    method: 'POST',
    data: { userId: userId, adminPassword: adminPassword },
    dataType: 'json',
    success: function(response) {
        if (response.success) {
            // Si la contraseña es correcta, mostrar la contraseña del usuario
            console.log("aqui entro",response); 
            showUserPassword(userId);
            $("#showPasswordModal").modal('hide'); // Ocultar el modal
        } else {
            // Mostrar el modal en caso de que esté oculto
            console.log("aqui no entro",response); 
            $("#showPasswordModal").modal('show');
            $("#passwordError").show(); // Mostrar mensaje de error si la contraseña es incorrecta
        }
    },
...
// Función para mostrar el modal y pasar el ID del usuario
function showModalAndPassUserId(userId) {
    $("#showPasswordModal").data('userId', userId).modal('show'); // Pasar el ID del usuario y mostrar el modal
}
...
<script>
// Script para limpiar el input al cerrar el modal
  $("#showPasswordModal").on('hidden.bs.modal', function () {
      $("#adminPassword").val(""); // Limpiar el valor del input
  });
</script>
